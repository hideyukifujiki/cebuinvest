...
import {
  AlertCircle, ArrowRight, Building2, Check, Coins,
  Laptop, Rocket, Users, Wifi,
} from "lucide-react";
...
import {
  type WorkStyle,
  getRelatedWorkStyles,
} from "@/lib/work-styles";

type Props = { style: WorkStyle };
...
export function WorkStyleTemplate({ style }: Props) {
  const Icon = iconMap[style.iconName];
  const related = getRelatedWorkStyles(style.slug);
  ...
  return (
    <>
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-10">
            <div className="lg:col-span-7">
              <div className="text-sm font-bold tracking-widest text-teal-400 mb-3">OVERVIEW</div>
              <h2 className="text-2xl lg:text-3xl font-extrabold tracking-tight leading-tight">
                {style.title}とは
              </h2>
              <p className="mt-6 text-ink-600 leading-relaxed text-base">{style.overview}</p>
            </div>
              ...
              <div className="bg-teal-50 rounded-2xl p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm mt-1 leading-relaxed">{style.scopeGuideline}</p>
                  </div>
                </div>
              </div>
              <div className="bg-sun-50 rounded-2xl p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm mt-1 leading-relaxed">{style.costGuideline}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* 対象 / 気をつけたいこと */}
      <section className="py-16 lg:py-24 bg-teal-50/50">
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <SectionHeading
            eyebrow="FIT & CAUTIONS"
            title={`${style.title}で気をつけたい人と、ケア事項`}
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <SectionHeading
            ...
            title="判断軸"
            lead="数字や肩書きだけで決めず、目的に沿って整える。"
            className="mb-12"
          />
          <div className="space-y-3">
            {style.selectionPoints.map((point, i) => (
              <div
                key={i}
      ...
      <section className="py-16 lg:py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
          <SectionHeading
            ...
            title={`${style.title}に向くエリア`}
            className="mb-12"
          />
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {style.recommendedAreas.map((area) => (
              <Link
                key={area.slug}
      ...
      {related.length > 0 && (
        <section className="py-16 lg:py-24 bg-teal-50/50">
          <div className="max-w-7xl mx-auto px-5 lg:px-8">
            <SectionHeading eyebrow="RELATED WORK STYLES" title="関連する働き方" className="mb-12" />
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {related.map((r) => (
                <Link
                  key={r.slug}
                  ...
                  className="card-hover bg-white rounded-2xl p-6 border border-slate-200"
                >
                  <div className="text-xs font-bold text-teal-400 tracking-widest">{r.eyebrow}</div>
      ...
      {/* 関連カテゴリーの記事 */}
      <RelatedArticles
        categorySlug={style.relatedGuideCategory}
        limit={3}
      ...
      {/* CTA */}
      <ConsultationCta
        heading={`${style.title}について、一緒に整理します。`}
        primaryLabel={`${style.title}を相談する`}
        secondary={{ label: "働くのトップへ", href: "/work" }}
      />

      <StickyCta label={`${style.title}を相談する`} />
    </>
  );
}
