...
import {
  ArrowRight, BookOpen, Briefcase, Building, Check, Clock,
  Coins, GraduationCap, Timer, Users, X,
} from "lucide-react";
...
export function StudyTopicTemplate({ topic }: Props) {
  const Icon = iconMap[topic.iconName];
  const related = getRelatedStudyTopics(topic.slug);
  ...
  return (
    <>
      ...
      {/* HERO */}
      <section
        className={`relative overflow-hidden bg-gradient-to-br ${topic.gradient} text-white`}
      >
        ...
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-10">
            <div className="lg:col-span-7">
              ...
              <p className="mt-6 text-ink-600 leading-relaxed text-base">
                {topic.overview}
              </p>
            </div>
            <div className="lg:col-span-5 space-y-4">
              <div className="bg-sky-50 rounded-2xl p-5">
                <div className="flex items-start gap-3">
                  <Timer className="h-5 w-5 text-navy-700 shrink-0 mt-0.5" />
                  <div>
                    <div className="text-xs font-bold text-navy-700">期間の目安</div>
                    <p className="text-sm mt-1 leading-relaxed">{topic.durationGuideline}</p>
                  </div>
                </div>
              </div>
              <div className="bg-sun-50 rounded-2xl p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-navy-700">予算の目安</div>
                    <p className="text-sm mt-1 leading-relaxed">{topic.budgetGuideline}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="py-16 lg:py-24 bg-sky-50">
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <SectionHeading
            eyebrow="FIT"
            title={`${topic.title}は、誰に向くか`}
            lead="判断軸を明確にして、自分に合うかを冷静にチェック。"
            className="mb-12"
          />
          <div className="grid md:grid-cols-2 gap-5">
            <div className="bg-white rounded-2xl p-6 lg:p-8 border border-slate-200">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-extrabold">こんな人に向く</h3>
              </div>
              <ul className="space-y-3">
                {topic.forWhom.map((item, i) => (
            ...
            <div className="bg-white rounded-2xl p-6 lg:p-8 border border-slate-200">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center">
                  <X className="h-5 w-5 text-ink-600" strokeWidth={3} />
                </div>
                <h3 className="text-lg font-extrabold">向かないかも</h3>
              </div>
              <ul className="space-y-3">
                {topic.notForWhom.map((item, i) => (
                  <li key={i} className="text-sm text-ink-700 leading-relaxed flex items-start gap-2">
                    <span className="text-ink-600 font-bold mt-0.5">・</span>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <SectionHeading
            ...
            title="選び方のコツ"
            lead="数字や肩書きだけで決めず、目的に沿った判断軸で。"
            className="mb-12"
          />
          <div className="space-y-3">
            {topic.selectionPoints.map((point, i) => (
              <div
                key={i}
      ...
      <section className="py-16 lg:py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
          <SectionHeading
            ...
            title={`${topic.title}に向くエリア`}
            className="mb-12"
          />
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {topic.recommendedAreas.map((area) => (
              <Link
                key={area.slug}
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-3xl mx-auto px-5 lg:px-8">
          ...
          <p className="mt-8 text-xs text-ink-600 text-center">
            学校情報・料金・入学条件は変更されることがあります。最新情報は各学校公式または認定エージェントにご確認ください。
          </p>
        </div>
      </section>
      ...
      {/* 関連カテゴリーの記事 */}
      <RelatedArticles
        categorySlug={topic.relatedGuideCategory}
        limit={3}
      ...
      {/* CTA */}
      <ConsultationCta
        heading={`${topic.title}について、一緒に整理します。`}
        primaryLabel={`${topic.title}を相談する`}
        secondary={{ label: "他の学び方を見る", href: "/study" }}
      />

      <StickyCta label={`${topic.title}を相談する`} />
    </>
  );
}
