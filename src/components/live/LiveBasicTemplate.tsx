...
import {
  AlertCircle, ArrowRight, Check, Coins, GraduationCap,
  Heart, Home as HomeIcon, ShieldCheck,
} from "lucide-react";
...
import {
  type LiveBasic,
  getRelatedLiveBasics,
} from "@/lib/live-basics";

type Props = { basic: LiveBasic };
...
export function LiveBasicTemplate({ basic }: Props) {
  const Icon = iconMap[basic.iconName];
  const related = getRelatedLiveBasics(basic.slug);
  ...
  return (
    <>
      <Breadcrumb
        ...
        bgClassName="bg-palm-50"
      />

      {/* HERO */}
      <section className={`relative overflow-hidden bg-gradient-to-br ${basic.gradient} text-white`}>
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-10">
            <div className="lg:col-span-7">
              <div className="text-sm font-bold tracking-widest text-palm-500 mb-3">OVERVIEW</div>
              <h2 className="text-2xl lg:text-3xl font-extrabold tracking-tight leading-tight">
                {basic.title}の全体像
              </h2>
              <p className="mt-6 text-ink-600 leading-relaxed text-base">{basic.overview}</p>
            </div>
            <div className="lg:col-span-5 space-y-4">
              <div className="bg-palm-50 rounded-2xl p-5">
                <div className="flex items-start gap-3">
                  <Icon className="h-5 w-5 text-navy-700 shrink-0 mt-0.5" />
                  <div>
                    <div className="text-xs font-bold text-navy-700">対象範囲</div>
                    <p className="text-sm mt-1 leading-relaxed">{basic.scopeGuideline}</p>
                  </div>
                </div>
              </div>
              <div className="bg-sun-50 rounded-2xl p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-navy-700">コスト感</div>
                    <p className="text-sm mt-1 leading-relaxed">{basic.costGuideline}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* 対象 / 注意点 */}
      <section className="py-16 lg:py-24 bg-palm-50/50">
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <SectionHeading
            eyebrow="FIT & CAUTIONS"
            title={`${basic.title}で気をつけたい人と、ケア事項`}
            lead="判断軸を明確にして、自分の状況に合わせて整える。"
            className="mb-12"
          />
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-2xl p-6 lg:p-8 border border-slate-200">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-extrabold">気をつけたいこと</h3>
              </div>
              <ul className="space-y-3">
                {basic.cautions.map((item, i) => (
                  <li key={i} className="text-sm text-ink-700 leading-relaxed flex items-start gap-2">
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-5xl mx-auto px-5 lg:px-8">
          <SectionHeading
            ...
            title="判断軸"
            lead="数字や肩書きだけで決めず、暮らしの目的に沿って整える。"
            className="mb-12"
          />
          <div className="space-y-3">
            {basic.selectionPoints.map((point, i) => (
              <div
                key={i}
                className="flex items-start gap-4 p-5 rounded-2xl border border-slate-200"
              >
                <div className="w-9 h-9 rounded-lg bg-palm-50 flex items-center justify-center shrink-0">
      ...
      <section className="py-16 lg:py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
          <SectionHeading
            ...
            title={`${basic.title}に向くエリア`}
            className="mb-12"
          />
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {basic.recommendedAreas.map((area) => (
              <Link
                key={area.slug}
      ...
      {related.length > 0 && (
        <section className="py-16 lg:py-24 bg-palm-50/50">
          <div className="max-w-7xl mx-auto px-5 lg:px-8">
            <SectionHeading eyebrow="RELATED LIVING BASICS" title="関連する暮らしの基本" className="mb-12" />
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {related.map((r) => (
                <Link
                  key={r.slug}
                  ...
                  className="card-hover bg-white rounded-2xl p-6 border border-slate-200"
                >
                  <div className="text-xs font-bold text-palm-500 tracking-widest">{r.eyebrow}</div>
      ...
      {/* 関連カテゴリーの記事 */}
      <RelatedArticles
        categorySlug={basic.relatedGuideCategory}
        limit={3}
      ...
      {/* CTA */}
      <ConsultationCta
        heading={`${basic.title}について、一緒に整理します。`}
        primaryLabel={`${basic.title}を相談する`}
        secondary={{ label: "暮らすのトップへ", href: "/live" }}
      />

      <StickyCta label={`${basic.title}を相談する`} />
    </>
  );
}
